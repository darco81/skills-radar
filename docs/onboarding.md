# Onboarding - get skills-radar running in 5 minutes
...
This creates `~/.config/skills-radar/config.yaml` with sensible defaults.
...
```bash
skills-radar search "audit my code for accessibility"
```
...
Add `skills-radar` to your project's `.mcp.json`:
...
Now that `skills-radar` knows your skills, you can stop preloading their full descriptions in every Claude Code session. Edit `~/.claude/settings.json`:
...
Claude should call `mcp__skills-radar__search_skills` with a relevant query and surface the top match (probably `perf-vue-runtime`). Then it can call `mcp__skills-radar__load_skill` to fetch the full SKILL.md.

## Troubleshooting

**`skills-radar: command not found`**
The package didn't install or your venv isn't activated. Run `which skills-radar`. If empty, re-install or activate the venv.

**`No SKILL.md files found`**
Your paths don't exist or they're empty. Run `skills-radar doctor` and check the ✓/✗ marks.

**Search returns irrelevant matches**
Either: (a) the relevant skill isn't in your indexed paths, or (b) its description is too vague. Check with `skills-radar list` whether the skill is indexed; if it is, improve its `description` and `when_to_use` fields.

**`DuplicateIDError` during index**
Two SKILL.md files share the same `name:`. skills-radar dedupes by trust tier + mtime, but this error suggests an older version. Update via `pip install -U skills-radar`.

**Claude Code doesn't seem to query skills-radar**
