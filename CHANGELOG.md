...
### Fixed - reranker scores the full indexed text (description + when_to_use)

Both reranker backends (`ollama`, `mlx`) scored `metadata.description` only, ignoring `when_to_use` even though it is half of what the embedder and BM25 index (`indexed_text = description + "\n\n" + when_to_use`). Every trigger phrase and sibling-disambiguation note an author bakes into `when_to_use` per `docs/writing-skills.md` was invisible to the rerank stage — so enabling the reranker (the path meant to *help* fuzzy/multilingual queries) actively discarded the richest ranking signal. The two backends now share a `_candidate_text()` helper that prefers the candidate `document` (the full indexed text the store already holds), falling back to `description` only when absent — making all three stages (embed, lexical, rerank) score the same text. Tests: `tests/test_reranker.py::test_ollama_rerank_scores_full_indexed_text_including_when_to_use` (a when_to_use-only phrase reaches the scorer prompt for a query that doesn't contain it).
